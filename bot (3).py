...
10. If the user asks for a chart/image, the bot generates it separately. Never claim an image was generated unless the bot actually sends one.
...
def generate_market_chart(klines, coin_name, ta, snapshot):
    ...
    # Neutral chart styling: strong geometry/line width keeps levels visible on Telegram.
    for i, (op, hi, lo, cl) in enumerate(zip(opens, highs, lows, closes)):
        up = cl >= op
        edge = "black" if up else "0.45"
        ax.vlines(i, lo, hi, color=edge, linewidth=1.15, zorder=2)
        body_height = max(abs(cl - op), max(cl * 0.00015, 1e-8))
        ax.add_patch(
            Rectangle(
                (i - 0.32, min(op, cl)),
                0.64,
                body_height,
                facecolor="white" if up else "0.45",
                edgecolor=edge,
                linewidth=0.9,
                zorder=2,
            )
        )
        av.bar(i, volumes[i], width=0.64, alpha=0.45)

    def sma(values, period):
        return [sum(values[i-period+1:i+1]) / period if i >= period-1 else None
                for i in range(len(values))]

    ax.plot(x, sma(closes, 20), label="SMA20", linewidth=2.0)
    ax.plot(x, sma(closes, 50), label="SMA50", linewidth=2.0)

    if ta:
        levels = [
            (ta.support, "Support 1", "-"),
            (ta.near_support, "Support 2", "--"),
            (ta.near_resistance, "Resistance 1", "--"),
            (ta.resistance, "Resistance 2", "-"),
        ]
        seen = set()
        for level, label, style in levels:
            if level is None:
                continue
            ...
            seen.add(key)
            ax.axhline(
                float(level), linestyle=style, linewidth=2.4,
                alpha=0.95, zorder=4
            )
            ax.annotate(
                f"{label}  ${float(level):,.2f}",
                xy=(len(x)-1, float(level)),
                xytext=(-8, 0),
    ...
    if snapshot:
        ax.axhline(
            float(snapshot.price), linestyle=":", linewidth=2.2,
            alpha=0.95, zorder=4
        )
        ax.annotate(
            f"Current  ${float(snapshot.price):,.2f}",
            xy=(len(x)-1, float(snapshot.price)),
            xytext=(-8, 14),
            textcoords="offset points",
            ha="right",
            va="bottom",
    ...
    ax.set_title(
        f"{coin_name} ({snapshot.symbol if snapshot else coin_name} ) — 1 Hour Binance Chart",
        fontsize=15,
        fontweight="bold",
    )
...
async def transcribe_voice_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    def _transcribe():
        response = groq_client.audio.transcriptions.create(
            ...
            prompt=(
                "The speaker may use Bengali, Banglish, Bengali phonetic spellings, "
                "crypto terms, coin names, Binance, Ethereum, Bitcoin, BTC, ETH, "
                "MUSE, TOTAL, TOTAL2, TOTAL3, support, resistance, RSI and SMA. "
                "Transcribe what was actually said; preserve Bengali words and crypto names."
            ),
...
async def process_text_query(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str) -> None:
    ...
    if resolved:
        ...
        try:
            ...
            klines = await binance_klines(symbol, "1h", 100)
    ...
    if wants_chart and resolved and klines:
        try:
            chart_buf = await asyncio.to_thread(
                generate_market_chart, klines, coin_name, ta, snapshot
            )
