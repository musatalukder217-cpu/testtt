...
CRYPTO_KEYWORDS = [
    ...
    "dominance", "bull", "bear", "pump", "dump", "airdrop", "tokenomics",
    "ক্রিপ্টো", "বিটকয়েন", "ইথেরিয়াম", "ব্লকচেইন", "কয়েন",
    "क्रिप्टो", "बिटकॉइन",
]
...
def is_crypto_query(text: str) -> bool:
    lowered = text.lower()
    if any(alias in lowered for alias in COIN_ALIASES):
        return True
...
def extract_symbol(text: str) -> Optional[tuple[str, str]]:
    lowered = text.lower()
    for alias, (symbol, name) in COIN_ALIASES.items():
        if re.search(rf"\b{re.escape(alias)}\b", lowered):
            return symbol, name
    # Fallback: a bare 3-6 letter uppercase ticker like "OP" or "ARB".
    match = re.search(r"\b([A-Za-z]{2,6})\b", text)
    if match and match.group(1).lower() not in {"the", "and", "for", "btc"}:
...
# ==================================================
# LANGUAGE DETECTION (lightweight, script-based)
# ==================================================
...
SYSTEM_PROMPT_TEMPLATE = """You are a crypto-market analysis assistant embedded in a Telegram bot.

STRICT RULES:
1. You ONLY discuss cryptocurrency, blockchain, crypto markets, trading, tokenomics,
   crypto security, crypto news, and directly related topics. If the user's query is
   unrelated, politely decline and say you can only help with crypto topics.
2. You must base factual claims (price, % change, highs/lows, volume, indicator values)
   ONLY on the data provided to you in the CONTEXT below. Never invent numbers.
3. If context data is missing for something the user asked, say so plainly instead of
   guessing.
4. This is not financial advice. When discussing possible market direction, present it
   as scenarios/analysis, not certainty or personalized investment advice.
5. Reply in this language: {language_name}. Match the user's language automatically.
6. Be concise and use clear structure (short sections/bullets) for analysis answers.
"""
...
async def require_approved(update: Update) -> bool:
    ...
    if row is None or row["status"] != "approved":
        ...
        key = "rejected" if status == "rejected" else "pending"
        await update.message.reply_text(ui_text(key, lang))
        return False
    return True
...
def format_user_row(row: sqlite3.Row) -> str:
    ...
    return f"{row['telegram_id']} | {uname} | {row['first_name']} | {row['status']}"
...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    row = ensure_user_record(update)
    lang = detect_language(update.message.text or "")
    if row["status"] == "approved":
        await update.message.reply_text(ui_text("welcome", lang))
    elif row["status"] == "pending":
        await update.message.reply_text(ui_text("pending", lang))
        await notify_admins(
            context,
            f"🆕 New access request:\n{format_user_row(row)}\n"
            f"Approve with: /approve {row['telegram_id']}",
        )
    else:
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not rate_limiter.allow(user_id):
        await update.message.reply_text(ui_text("rate_limited", lang))
        return

    if not is_crypto_query(text):
        await update.message.reply_text(ui_text("not_crypto", lang))
        return
    ...
    ta: Optional[TechnicalSummary] = None
    ...
    if resolved:
        ...
        try:
            ...
            klines = await binance_klines(symbol, interval="1h", limit=100)
    ...
    try:
        ai_reply = await ask_groq(system_prompt, context_block)
    except Exception:
    ...
    await reply_long(update, ai_reply or "No response generated.")
