...
def generate_market_chart(klines, coin_name, ta, snapshot, timeframe_label="1 Hour"):
    import matplotlib
    matplotlib.use("Agg")
    ...
    # Each market level has its own clearly visible horizontal-line color.
    if ta:
        levels = [
    ...
    ax.set_title(
        f"{coin_name} ({symbol_text}) — {timeframe_label} Binance Chart",
    ...
    if wants_chart and resolved and klines:
        try:
            chart_buf = await asyncio.to_thread(
                generate_market_chart, klines, coin_name, ta, snapshot, timeframe_label
            )
